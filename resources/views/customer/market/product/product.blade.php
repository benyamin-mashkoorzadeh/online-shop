...
@section('content')
                        ...
                        <section class="col-md-4">
                            <section class="content-wrapper bg-white p-3 rounded-2 mb-4">
                                <section class="product-gallery">
                                    @php
                                        $imageGalleries = $product->images()->get();
                                        $images = collect();

                                        $images->push($product->image);

                                        foreach ($imageGalleries as $imageGallery) {
                                            $images->push($imageGallery->image);
                                        }
                                            ...
                                            <p>
                                                ...
                                                <select name="guarantee" id="guarantee" class="p-1">
                                                    @foreach($guarantees as $key => $guarantee)
                                                        <option data-guarantee-price="{{ $guarantee->price_increase }}"
                                                                @if($key == 0) selected
                                                                @endif value="{{ $guarantee->id }}">{{ $guarantee->name }}</option>
                                                    @endforeach
                                                </select>
                                            </p>
                                        ...
                                        @auth
                                            @if($product->user->contains(auth()->user()->id))
                                                <section class="product-add-to-favorite position-relative"
                                                    ...
                                                    <button
                                                        type="button"
                                                        class="btn_favorite btn btn-light btn-sm text-decoration-none"
                                                        data-url="{{ route('customer.market.add-to-favorite', $product) }}"
                                                        data-bs-toggle="tooltip" data-bs-placement="left"
                                                        title="حذف از علاقه مندی">
                                                        <i class="fa fa-heart text-danger"></i> حذف از علاقه مندی
                                                    </button>
                                                </section>
                                            @else
                                                <section class="product-add-to-favorite position-relative"
                                                ...
                                                <section>
                                                    <section class="cart-product-number d-inline-block ">
                                                        <button class="cart_number cart-number-down" type="button">-
                                                        </button>
                                                        <input id="number" name="number" type="number" min="1" max="5"
                                                               step="1"
                                                               value="1"
                                                               readonly="readonly">
                                                        <button class="cart_number cart-number-up" type="button">+
                                                        </button>
                                                    </section>
                                                </section>
                                                <p class="mb-3 mt-5">
                        ...
                        <section class="col-md-3">
                            <section class="content-wrapper bg-white p-3 rounded-2 cart-total-price">
                                ...
                                <section class="">
                                    ...
                                    @else
                                        <button id="next-level" class="btn btn-secondary disabled d-block">محصول
                                            ناموجود می‌باشد
                                        </button>
                                    @endif
                                </section>
                            ...
                            <section class="product-comments mb-4">
                                            ...
                                            <section class="modal-content">
                                                ...
                                                @guest
                                                    <section class="modal-body">
                                                        <p>کاربر گرامی لطفآ برای ثبت نظر وارد حساب کاربری خود شوید</p>
                                                        <p>
                                                        ...
                                                        <form class="row"
                                                            ...
                                                            <section class="col-12 mb-2">
                                                                ...
                                                                <textarea class="form-control form-control-sm"
                                                                          id="comment"
                                                                          name="body" placeholder="دیدگاه شما ..."
                                                                          rows="4"></textarea>
                                                            </section>
                                ...
                                @foreach($product->activeComments() as $activeComment)
                                    <section class="product-comment">
                                        ...
                                        @foreach($activeComment->answers()->get() as $answerComment)
                                            <section class="product-comment">
                                                <section class="product-comment-header d-flex justify-content-start">
                                                    <section
                                                        class="product-comment-date">{{ jalaliDate($answerComment->created_at) }}</section>

                                                    @php
                                                        $author = $answerComment->user()->first();
                                                    @endphp
                                                    <section class="product-comment-title">
                                                        @if(empty($author->first_name) && empty($author->last_name))
                                                            ناشناس
    ...
    <section class="position-fixed p-4 flex-row-reverse"
             style="z-index: 9999999999; left: 0; top: 3rem; width: 26rem; max-width: 80%">

        <div class="toast" data-delay="70000" role="alert" aria-live="assertive" aria-atomic="true">
    ...
    <script>
        ...
        function bill() {
            ...
            if ($('input[name="color"]:checked').length != 0) {
                selected_color = $('input[name="color"]:checked')
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.status == 1) {
                        ...
                        $(element).html('<i class="fa fa-heart text-danger"></i>')
                        $('.btn_favorite').html('<i class="fa fa-heart text-danger"></i> ')
                        $('.btn_favorite').append('حذف از علاقه مندی')
                        ...
                        $('.btn_favorite').html('<i class="fa fa-heart"></i> ')
                        $('.btn_favorite').append('افزودن به علاقه مندی')

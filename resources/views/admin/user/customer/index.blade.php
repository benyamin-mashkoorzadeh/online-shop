...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item font-size-12 active"> مشتریان</li>
        </ol>
    </nav>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        @foreach($users as $key => $user)
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" id="{{ $user->id }}-active"
                                           onchange="changeActive({{ $user->id }})"
                                           @if ($user->activation === 1) checked
                                           @endif data-url="{{ route('admin.user.customer.activation', $user->id) }}">
                                </td>
                                <td>
                                    <input type="checkbox" id="{{ $user->id }}"
                                           onchange="changeStatus({{ $user->id }})"
                                           @if ($user->status === 1) checked
                                           @endif data-url="{{ route('admin.user.customer.status', $user->id) }}">
                                </td>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status) {
                        if (response.checked) {
                            element.prop('checked', true)
                            successToast('فعال سازی مشتری با موفقیت فعال شد')
                        } else {
                            element.prop('checked', false)
                            successToast('فعال سازی مشتری با موفقیت غیرفعال شد')
                        }
                    } else {
                        element.prop('checked', elementValue)
                        errorToast('هنگام ویرایش مشکلی به وجود آمده است')

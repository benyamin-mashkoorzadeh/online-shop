                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" id="{{ $comment->id }}"
                                           ...
                                           @endif data-url="{{ route('admin.content.comment.status', $comment->id) }}">
                                </td>
                                <td class="max-width-16-rem text-left">
                                    <a href="{{ route('admin.content.comment.show', $comment->id) }}" class="btn btn-info btn-sm"><i
                                            class="fa fa-eye"></i> نمایش</a>

                                        @if($comment->approved == 1)
                                        <a href="{{ route('admin.content.comment.approved', $comment->id) }}" type="submit" class="btn btn-warning btn-sm text-white"><i class="fa fa-clock"></i>
                                            عدم تایید
                                        </a>
                                        @else
                                        <a href="{{ route('admin.content.comment.approved', $comment->id) }}" type="submit" class="btn btn-success btn-sm text-white"><i class="fa fa-check"></i>
                                            تایید
                                        </a>
                                        @endif
                                </td>

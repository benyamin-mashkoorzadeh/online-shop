...
class DiscountController extends Controller
{
    ...
    public function commonDiscountStore(CommonDiscountRequest $request)
    {
        ...
        return redirect()->route('admin.market.discount.commonDiscount')->with('swal-success', 'کد تخفیف با موفقیت ثبت شد');
    }
    ...
    public function commonDiscountUpdate(CommonDiscountRequest $request, CommonDiscount $commonDiscount)
    {
        ...
        return redirect()->route('admin.market.discount.commonDiscount')->with('swal-success', 'کد تخفیف با موفقیت ویرایش شد');
    }
    ...
    public function amazingSaleStore(AmazingSaleRequest $request)
    {
        ...
        $realTimestampStart = substr($request->start_date, 0, 10);
        $inputs['start_date'] = date("Y-m-d H:i:s", (int)$realTimestampStart);
        $realTimestampEnd = substr($request->start_date, 0, 10);
        $inputs['end_date'] = date("Y-m-d H:i:s", (int)$realTimestampEnd);
        $amazingSale = AmazingSale::create($inputs);

        return redirect()->route('admin.market.discount.amazingSale')->with('swal-success', 'کد تخفیف شگفت انگیز با موفقیت ثبت شد');
    ...
    public function amazingSaleUpdate(AmazingSaleRequest $request, AmazingSale $amazingSale)
    {
        ...
        return redirect()->route('admin.market.discount.amazingSale')->with('swal-success', 'کد تخفیف شگفت انگیز با موفقیت ویرایش شد');
    ...
    public function amazingSaleDestroy(AmazingSale $amazingSale)
    {
        ...
        return redirect()->route('admin.market.discount.amazingSale')->with('swal-success', 'کد تخفیف شگفت انگیز با موفقیت حذف شد');
    }
}
